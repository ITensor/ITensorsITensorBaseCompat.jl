# Compatibility shims bridging the legacy `ITensors.jl` API to the next-gen
# `ITensorBase.jl` backend.
#
...
# Functions consumers add methods to for their own types (`scalartype`, `contract`,
# `inner`, `datatype`). Rather than extend the upstream generics, this layer owns these:
# `scalartype` falls back to ITensorBase for tensors/arrays; `contract`/`inner` are
# operations with their own base methods defined by the consumer. The per-file
# method definitions extend these module-owned functions. (`scalartype`/`datatype`
# live in the `NDTensors` submodule and are imported into this one.)
function inner end
...
# integer indices into `tensors`. Consumers add the network-level `contract` methods.
...
# unchanged. Consumers nest these calls (e.g. `uniqueinds(uniqueinds(...), ...)`), so the
...
# Singular forms: the one common / one unique index. `noncommonind` is used
...
# `prime` / `noprime`: ITensorBase primes an `Index`; this layer also primes whole ITensors
...
# shadow `LinearAlgebra.tr`, which consumers still call on plain matrices.
...
# Covers the one- and two-site gates applied to states without pre-existing primes.
...
#
# `array` exposes the plain unnamed array. (`scalartype` / `datatype` / `data` live in
...
# The operator / named-state system (`op` / `state`) is vendored separately in the
# `SiteTypes` submodule (`SiteTypes.jl`), included right after this file.
...
# stores tags as a `Dict{String, String}`. For the legacy flat-tag usage (site-type
# labels, link names), we store each token as a keyed tag with empty value, and
# `hastags` checks membership. (A fuller tag-compat story is a follow-up.)
